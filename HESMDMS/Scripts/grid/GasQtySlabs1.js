$(document).ready(function () {
    ...
    function loadGasQtySlabs() {
        $("#GasQtySlabGrid").dxDataGrid({
            dataSource: "/Admin/ATGLRates/GetGasQtySlabs",
            keyExpr: "ID",
            paging: { pageSize: 10 },
            columns: [
                { dataField: "ID", caption: "ID", width: 50, visible: false },
                { dataField: "GroupName", caption: "Group Name" },
                { dataField: "GasqtySlab1", caption: "Gas Quantity Slab 1", format: "#,##0.000" },
    ...
    $("#gasQtySlabForm").submit(function (e) {
        ...
        let gasQtySlab = {
            ...
            GasqtySlab1: parseFloat($("#GasQtySlab1").val()).toFixed(3),
            Date: new Date().toISOString()
        };

        let url = gasQtySlab.ID == 0 ? "/Admin/ATGLRates/AddGasQtySlab" : "/Admin/ATGLRates/UpdateGasQtySlab";
    ...
    function editGasQtySlab(data) {
        ...
        $("#GasQtySlab1").val(parseFloat(data.GasqtySlab1).toFixed(3));
    ...
    function deleteGasQtySlab(id) {
        if (confirm("Are you sure you want to delete this record?")) {
            $.post("/Admin/ATGLRates/DeleteGasQtySlab", { id: id }, function (response) {
                alert(response.message);
                loadGasQtySlabs();
                $("#GasQtySlabGrid").dxDataGrid("instance").refresh();
            });
